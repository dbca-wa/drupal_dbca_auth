...
class WebserverAuthSettingsForm extends ConfigFormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $form['advanced']['site_access']['block_access_redirect_url'] = [
      ...
      '#description' => $this->t('Provide optional URL where blocked users should be redirected to.'),
    ];
...
//    ];
//
//    $form['advanced']['account_modification'] = [
//      '#type' => 'textarea',
//      '#title' => $this->t('User account modification'),
//      '#default_value' => $config->get('account_modification'),
//      '#description' => $this->t('Modify user accounts at the time of creation. Use PHP code (enclosed in <code>&lt;?php</code> and <code>?&gt;</code>). The variable <code>\$account</code> is available as in <a href=\"http://api.drupal.org/api/function/hook_user/7\">hook_user(\'insert\',...)</a>. Changes to the user object must be explicitly saved to the database to be made permanent.'),
//    ];
//
